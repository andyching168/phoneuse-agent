...
## ⚠️ 實作心得（2026-03-26 重要更新）

### JSON 標記點會比畫面上看到的少？這是正常的！

**重要觀念**：
- JSON markers.json 裡的標記點只是**最常用的動作**，不會涵蓋畫面上所有按鈕
- 遇到 JSON 沒有標記的按鈕時，先用 `screen_overview` 確認目前畫面
- 找到畫面上最接近的描述，嘗試在 JSON 中找對應的狀態名稱

### 狀態識別流程

1. **先用 `screen_overview` 看目前畫面**：了解畫面上有哪些元素
2. **用 `list_states --json markers.json`**：列出所有可能的狀態
3. **用 `screen_overview` 的輸出比對 description**：找最接近的狀態
4. **用 `list_markers "<懷疑的狀態名>" --json markers.json`**：查看該狀態的 markers
5. **如果 markers 剛好有你要的動作**：直接用 `run_marker_follow`
6. **如果沒有**：用 full 模式輔助，或估算座標點擊

### 實戰範例：公主連結行程表掃蕩

```
畫面:「地下城掃蕩確認」- 確定要掃蕩地下城的所有戰鬥獲得獎勵嗎？
     取消 / OK / 簡單 / 設定清單 / 全部一鍵執行

分析:
- screen_overview 看到「地下城掃蕩確認」
- list_states 沒有完全一樣的狀態名
- 但有「經驗值/瑪那冒險掃蕩確認」description 說「瑪那和經驗值在掃蕩時會各有一輪」
- 點擊 OK → 前往「行程表經驗值/瑪那冒險掃蕩結果」
- 結果顯示 EXP +14,144,000 → 確認是正確狀態！
```

### 當 JSON 狀態和實際畫面不完全匹配時

1. **同名稱不同內容**：可能是同一狀態的兩輪（如 EXP 掃蕩 → 瑪那掃蕩）
2. **畫面元素相似**：用相同的 markers 嘗試，通常座標是通用的
3. **描述是關鍵線索**：state description 會提示這個狀態的用途

> 💡 **核心原則**：不要糾結於 JSON 狀態名和畫面元素 100% 匹配。重點是找到**最接近的狀態**，用它的 markers 嘗試，觀察結果是否符合預期。
